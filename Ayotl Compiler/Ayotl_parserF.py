...
import codecs
import re
from Ayotl_scanner import tokens
from Complementos.CuboSemantico import CuboSemantico
from Complementos.TablaVariables import TablaVariables
from Complementos.Cuadruplos import Cuadruplos
#from Complementos.MaquinaVirtual import MaquinaVirtual
...
def p_exp(p):
	...
	if(len(p)==2):
		...
		OperIzq = getTypeUniversal(p[1])
		OperDer = getTypeUniversal(p[3])
		temp=cubo.checkType(OperIzq,OperDer,'+')
...
def p_funcionLL(p):
	...
	if(dirFunciones.getFnType(funcNameLL[len(funcNameLL)-1])=='INT'):
		tablaTemp.addVar('INT','t'+str(tempCont))
		quads+=[('=',funcNameLL[len(funcNameLL)-1],tablaTemp.getDireccionMem('t'+str(tempCont)))]
		p[0]=tablaTemp.getDireccionMem('t'+str(tempCont))
		tempCont+=1
	
	elif(dirFunciones.getFnType(funcNameLL[len(funcNameLL)-1])=='FLOAT'):
		tablaTemp.addVar('FLOAT','t'+str(tempCont))
		quads+=[('=',funcNameLL[len(funcNameLL)-1],tablaTemp.getDireccionMem('t'+str(tempCont)))]
		p[0]=tablaTemp.getDireccionMem('t'+str(tempCont))
		tempCont+=1
